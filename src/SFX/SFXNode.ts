...
export class SFXNode {
  ...
  _getPanner(data: SFXData, options?: SFXPlayOptions) {
    if (data.is3dSound) {
      ...
      return Audio.api.createPannerNode(pannerData);
    }
    return false;
  }
  ...
  play(options?: SFXPlayOptions) {
    ...
    const master = Audio.api.createGain();
    ...
    if (options?.effects) {
      ...
      const sourceGain = Audio.api.createGain();
    ...
    source.onended = () => {
      ...
      for (const node of disconnectNodes) {
        node.disconnect();
        node.dispatchEvent(Audio.api._dissconectEvent);
      }
    };
